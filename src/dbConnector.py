...
class dbJob():
    ...
    def set_primary_key(self, targetTableName):
        ...
        '''
        self.logging.info(f"SET PRIMARY KEY FOR TABLE {targetTableName}")
        ...
        with self.engine.connect() as conn:
            ...
            self.logging.info(f"SET PRIMARY KEY FOR TABLE {targetTableName}: SUCCESSFUL!")
            conn.close()
        return
    ...
    def getData_slice(self, arg_choice, tableName, sort_by = "id_date", is_exact = False):
        ...
        if is_exact == False:
            ...
            # self.logging.info(f"DEBUGGING ------- {getData_query}")
    ...
    def insertData(self, dataTable, targetTableName, dtypes = None):
        ...
        if check_table == True and len(dataTable) != 0:
            self.logging.info("---------------------------------------")
            self.logging.info(f"INSERTING DATA INTO {targetTableName}")
            self.logging.info("CHECKING COLUMNS DIFFERENCE")
            ...
            if len(list_diff) != 0: # if there is any new columns, create T-SQL type for columns
                self.logging.info("APPEAR COLUMNS DIFFERENCE, ADDING COLUMN(s)")
                ...
                for new_col_idx in range(len(list_diff)):
                    self.logging.info(f"--------- ADDING COLUMN(s) {list_diff[new_col_idx]}")
            ...
            if len(listNew_idDate) != 0:
                self.logging.info("NO OVERLAP OF PRIMARY KEY, INSERTING DATA INTO TABLE")
                listNew_idDate.sort()
                # self.logging.info(f"DEBUGGING --- {listNew_idDate}")
                ...
                self.logging.info("FOUND OVERLAP OF PRIMARY KEY")
            
            # update if has new columns
            is_diffData, self.data_checkDict = self.compareDataRow(dataTable, targetTableName)
            if len(list_diff) != 0: 
                self.logging.info("APPEAR NEW COLUMN(S), INSERTING NEW COLUMN(S)")
                for new_col in list_diff:
                    self.updateData(targetTableName, dataTable['id_date'].values[0], 
                                    new_col, dataTable[new_col].values[0])
            elif len(list_diff) == 0:
                self.logging.info(f"NO NEW COLUMN TO PUSH")
                
            # update if has different data
            if is_diffData == True:
                self.logging.info("FOUND DIFFERENT IN DATA, UPDATING COLUMNS DATA")
                for column, value in self.data_checkDict.items():
                    self.logging.info(f"------ UPDATING COLUMNS {column}")
                    self.updateData(targetTableName, dataTable['id_date'].values[0], 
                                    column, value)
            else:
                self.logging.info(f"ALL DATA VALUE IS THE SAME FOR id_date {dataTable['id_date'].values[0]}")
                   
            self.logging.info(f"INSERT DATA INTO {targetTableName} FINISH")
            self.logging.info("---------------------------------------")
            
        elif check_table == False: # if not > creating new table
            self.logging.info(f"TABLE {targetTableName} DOES NOT EXIST IN DATABASE {self.database}")
            self.logging.info(f"CREATING NEW TABLE {targetTableName}")
    ...
    def insertData_VBMADanhMuc(self, dataTable, targetTableName, dtypes = None, fromsource = "VBMA"):
        ...
        if check_table == True:
            self.logging.info("---------------------------------------")
            self.logging.info(f"INSERT DATA INTO {targetTableName}")
            self.logging.info("CHECKING COLUMNS DIFFERENCE")
            ...
            if len(list_diff) != 0: # if there is any new columns, create T-SQL type for columns
                self.logging.info("APPEAR COLUMNS DIFFERENCE, ADDING COLUMNS")
                for new_col in list_diff:
                    type_ = dataTable[new_col].dtypes
                    if type_ == "float64":
                        list_diff_type.append("FLOAT")
            ...
            if fromsource == "VBMA":
                ...
                delete_row = list()
                self.logging.info("CHECKING UNIQUE ngay_TCPH")
            ...
            self.logging.info(f"DATA PUSH \n {dataTable}")
            if len(dataTable) > 0:
                self.logging.info(f"THERE ARE {len(dataTable)} NEW ROW(S) TO PUSH")
                dataTable.to_sql(targetTableName, self.engine.connect(), index=False, if_exists = 'append',
                                        dtype = dtypes, chunksize=20000)
                self.logging.info(f"INSERT DATA INTO {targetTableName} SUCCESSFUL")
                self.logging.info("---------------------------------------")
            else:
                self.logging.info("NO NEW DATA TO PUSH")
    ...
    def deleteData(self, table, condition):
        ...
        """
        self.logging.info(f"DELETE ROW id_date = {condition} IN TABLE {table}")
    ...
    def createNewTable(self, dataTable, targetTableName, dtypes = None, force_to_float = True):
        ...
        """
        self.logging.info(f"CREATE/REPLACE TABLE DATA: {targetTableName}")
        # Push table to sql
        ...
        self.logging.info(f"CREATE/REPLACE TABLE DATA {targetTableName} SUCCESSFUL")
        self.logging.info("---------------------------------------")
        return
    ...
    def replaceTable(self, dataTable, targetTableName, nvarcharCols = None):
        ...
        """
        self.logging.info(f"REPLACE TABLE DATA: {targetTableName}")
        if nvarcharCols is not None:
            ...
            dataTable.to_sql(targetTableName, self.engine.connect(), index = False,
                                if_exists = 'replace', chunksize=20000)
        self.logging.info(f"REPLACE TABLE DATA {targetTableName} SUCCESSFUL")
        self.logging.info("---------------------------------------")
        return
    ...
    def updateData(self, targetTableName, id_date_cond, update_column, update_value):
        ...
        """
        self.logging.info("UPDATING DATA")
        assert type(update_column) is str
        self.logging.info(f"NEW VALUE AT {update_column} -- {update_value} -- TYPE {type(update_value)}")
        
        if (update_value is None or pd.isnull(update_value) or update_value  == np.nan):
            self.logging.info("CASE 1 -- NAN")
            self.logging.info("CHANGE NAN VALUE TO COMPATIBLE VALUE")
            ...
            self.logging.info("CASE 2 -- INT OR FLOAT")
            update_query = f"UPDATE [dbo].[{targetTableName}] \
                            SET [{update_column}] = {update_value} \
                            WHERE id_date = {id_date_cond}"
        else:
            self.logging.info("CASE 2 -- STRING")
            update_query = f"UPDATE [dbo].[{targetTableName}] \
                            SET [{update_column}] = '{update_value}' \
                            WHERE id_date = {id_date_cond}"
    ...
    def compareDataRow(self, newTable, targetTableName):
        ...
        """
        self.logging.info("COMPARING OLD DATA WITH NEW DATA")
    ...
    def compareProcess(self, oldTable, newTable, is_check_date = True, **kwargs):
        ...
        if len(oldTable) != 0:
            ...
            # self.logging.info(f"DEBUGGING ---- CHECKLIST DATA EQUAL \n {isDataEqual_checkList}")
        is_diff = False
        if 1 in isDataEqual_checkList:
            is_diff = True
        # self.logging.info(f"DEBUGGING ---- IS_DIFF \n {is_diff}")
    ...
    def _cleanColAllNan(self, tableName):
        self.logging.info(f'----- CLEARING NAN COLUMNS TABLE {tableName} AND REPLACE')
        tableCache = self.getData(tableName)
        tableCache = tableCache.dropna(axis=1, how='all')
        self.replaceTable(tableCache, tableName)
        self.logging.info(f'----- CLEARING NAN COLUMNS TABLE {tableName} AND REPLACE: DONE')
        return
    ...
    def deleteColumnAllNan(self, tableName = None, isMassClean = False):
        ...
        if tableName is None and isMassClean == True:
            self.logging.info(f"MASS CLEAN MODE BEGIN, CLEAN ALL SPARE COLUMNS IN EVERY DATA TABLE OF THIS DATABASE {self.database}")
            for table_name in self.tableInDB:
                self._cleanColAllNan(table_name)
        elif tableName is not None and isMassClean == False:
            self.logging.info("CLEAN NAN COLUMNS FOR {tableName} ONLY")
            self._cleanColAllNan(tableName)
        self.logging.info("CLEARING SPARE COLUMN: DONE")
        return
    ...
    def deleteRowAllNan(self, tableName, skipColumns = None):
        ...
        """
        self.logging.info(f"DELETE ROW CONTAIN ALL NAN IN TABLE {tableName}")
        ...
        self.logging.info(f"DELETE ROW CONTAIN ALL NAN IN TABLE {tableName}: DONE")
        return 
